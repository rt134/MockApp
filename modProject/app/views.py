...
def car_payment_view(request):
	...
	if requests.get(User_url+context['email_id']).json() == []:
		ur = {"email_id": context['email_id'],"fname": context['fname'],"lname": context['lname'],"username": context['uname'],
			"age": context['age'],"gender": context['gender'],"contact": context['contact'],"dLicense": context['licenseno']}
...
def mech_payment_view(request):
	...
	if requests.get(ur).json() == []:
		ur = {"email_id": context['email_id'],"fname": context['fname'],"lname": context['lname'],"username": context['uname'],
			"age": 20,"gender": 'NA',"contact": context['contact'],"dLicense": 'NA'}
		usrres1 = requests.post(User_url,json=ur)
